...
votes = Table('votes', post_meta,
    ...
    Column('start_timestamp', DateTime),
    Column('end_timestamp', DateTime),
    Column('vote_resul', String(length=20)),
...
def upgrade(migrate_engine):
    ...
    pre_meta.tables['votes'].columns['vote'].drop()
    post_meta.tables['votes'].columns['vote_resul'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    pre_meta.tables['votes'].columns['vote'].create()
    post_meta.tables['votes'].columns['vote_resul'].drop()
